Model5 = "
functions{
// discrete weiull distr.
 real discrete_weibull_lpdf(real y, real x, real alpha, real gamma){
    real lp =  y * log(exp(-gamma*((x^alpha -1)/alpha)) - exp(-gamma*((x+1)^alpha -1)/alpha));
    return lp;
  }

// prior function
    real alpha_prior_lpdf(real alpha, real theta, real max, real size){
    
    real lp =  (size-1) * log1m_exp(-(max^alpha-1^alpha)/(alpha/theta)) + weibull_lpdf(max|alpha,(alpha/theta)^(1/alpha)) -log1m_exp(weibull_lcdf(1|alpha,(alpha/theta)^(1/alpha)));
    return lp;
    }     
}
data {
int N;
real<lower=0> x_contact[N];
real<lower=0> y_case00[N];
real<lower=0> y_case10[N];
}

parameters{
real<lower=0> alpha[2];
real<lower=0> beta[2];
}

model{ 
target += normal_lpdf(beta|0,10);
for(n in 1:2)
target += alpha_prior_lpdf(alpha[n] | 0.6, 700, 691)/10;
for(n in 1:N)
target += discrete_weibull_lpdf(y_case00[n] | x_contact[n], alpha[1], beta[1]) + discrete_weibull_lpdf(y_case10[n] | x_contact[n], alpha[2], beta[2]); // 1/log(N) * discrete_weibull_lpdf(y_case[n] | x_contact[n], alpha, beta);
}

generated quantities{
vector[N] llk;


for(n in 1:N)
llk[n] = discrete_weibull_lpdf(y_case00[n] | x_contact[n], alpha[1], beta[1]) + discrete_weibull_lpdf(y_case10[n] | x_contact[n], alpha[2], beta[2]);
}
"