...
beta ~ normal(0,10);
for(n in 1:N)
target += discrete_weibull_lpdf(y_case00[n] | x_contact[n], alpha[1], beta[1]) + discrete_weibull_lpdf(y_case10[n] | x_contact[n], alpha[2], beta[2]); // 1/log(N) * discrete_weibull_lpdf(y_case00[n] | x_contact[n], alpha[1], beta[1]) + 1/log(N) * discrete_weibull_lpdf(y_case10[n] | x_contact[n], alpha[2], beta[2]);
}
